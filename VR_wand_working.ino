 ...
 * 3   ------->     RX
 */
VR myVR(3,4);    // 2:TX 3:RX, you can choose your favourite pins.
...
myVR.load(uint8_t (4));
//Others
myVR.load(uint8_t (5)); //Incendio
myVR.load(uint8_t (6)); //Lumos
myVR.load(uint8_t (7)); //Nox
myVR.load(uint8_t (8)); //Kedavra
Serial.println("Incendio, Lumos, Nox, and Avada Kadavra loaded");
...
if(ret>0){
  if(buf[1]==0){
    digitalWrite(incendio,LOW);
    digitalWrite(lumos,LOW);
    digitalWrite(nox,LOW);
    digitalWrite(avada,LOW);
    Serial.println("TEST");
    myVR.clear();
    myVR.load(uint8_t (1));
    myVR.load(uint8_t (2));
  ...
  if(buf[1]==4){
    digitalWrite(avada,HIGH);
    Serial.println("Avada Kadavra!");
    delay (1000);
    digitalWrite(avada,LOW);
    myVR.clear();
    myVR.load(uint8_t (1));
